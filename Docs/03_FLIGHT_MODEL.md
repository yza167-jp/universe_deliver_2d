...
T-120 将白噪星工作值固定为数据表中的 `gravity_scale = 1.28`。飞行环境以当前
基准 `200 px/s² × 1.28 = 256 px/s²` 得到轨道、冰原、冰裂谷、极光占位、
地下档案与着陆六个 profile 的统一重力权威；各段只改变重力混合、空气密度、
阻力和终端下降保护，不在控制器内散落第二套重力数字。有限重力补偿仍为默认
`0.75`，玩家必须用俯仰和推力提前拉起，路线不会提供隐形自动驾驶。

白噪路线的局部冰裂谷在 `10000–17000 m` 以船体实际 Y 选择上层快速、中层常规
或下层观景通道。三条通道有可见分隔碰撞、各自安全重试 Y 和统一汇合距离；
分流不复制三份完整关卡，也不改变 M0 飞船积分、倒车、Boost 或碰撞模型。

### T-121 白噪电磁暴雪

`data/tuning/white_noise_storm_profile_m1.tres` 是白噪危险唯一参数源。当前固定
路线窗口为 `17000–23000 m`，预警/活跃/恢复的最短时间分别为
`2.4 / 12.0 / 2.8 s`，路线门槛分别为 `17700 / 22000 / 23000 m`。
`WhiteNoiseInterferenceModel` 使用单调最大路线距离推进
`CLEAR → WARNING → ACTIVE → RECOVERY`；阶段必须同时满足最短时间和距离门槛，
因此短距离倒车不会回滚天气，Boost 也不会把核心预警压成单帧。

活跃期每 `4.0 s` 产生一次基础 `18` 高压伤害与 `3` 满穿透货损请求；按最短
`12.0 s` 穿越时名义发生三次，停留更久则继续按间隔脉冲。控制器只调用
`FlightLabShip.apply_high_voltage_damage()`：
缩放后仍进入 D-073 的普通护盾优先结算，只有伤害穿透护盾时才按穿透比例影响
船体与货物。已安装 `module_high_voltage_shielding` 时，干扰强度直接乘模块
Resource 的 `0.45`，高压伤害与货损请求直接乘 `0.60`；仅拥有未安装时均为
`1.0`。暴雪配置不复制这两个倍率，也不建立第二条护盾资源。

干扰基础强度为 `0.86`、可见度压力为 `0.78`。它只降低局部能见度、延迟非核心
分流标签刷新并改变档案信标清晰度；速度、高度、护盾、船体、燃料、货物和危险
状态持续可读，玩家 throttle/brake/pitch/Boost 与 Input Map 从不被修改。
路线提示在暴雪中保留连续航标，高对比模式强化冰层边界和黄色引导，慢动作辅助
仅在预警期把时间倍率暂调为 `0.55`。帮助或系统暂停冻结危险计时与伤害；检查点、
F6、离开窗口和场景释放都确定恢复状态与原时间倍率。
...
| `planet_gravity` | 140–280 px/s² | 赤砂星基准约 180–210；白噪当前为 256 |
