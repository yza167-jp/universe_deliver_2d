...
地表参考高度与真实碰撞必须分开。HUD 可在空间接近和高层大气使用压缩的
高度基准，但星系外缘、陨石带、近轨道、大气边缘和雷暴层不得因此生成地面
视觉或 `StaticBody2D`；赤砂星当前只从低层云开始逐步启用可见且可碰撞的
地表，避免玩家在仍然看见完整星球或曲面地平线时撞到无来源的平直边界。
...
赤砂星峡谷雷达不是敌人 AI。固定扇区只按高度、路线位置和地形遮挡显式积分
锁定风险：HUD 必须同时显示当前高度和可避免锁定的安全高度。持续低飞或进入
遮挡不会受罚；高空完成锁定时每个检查点只触发一次轻量锁定脉冲，当前扣除
`12` 点护盾/船体伤害与 `2` 点货物完整度，并用状态文本和轻微镜头反馈说明
原因。低飞课程结束后清空锁定压力，保留至少约 `2950 m` 的抬升、减速和
对准着陆台缓冲，不把雷达路线直接接到接地判定。
...
“距着陆点 / 速度 / 高度”；进入雷达段时同一区域显示安全高度上限。按一次
