...
func run() -> Array[String]:
	...
	_test_missing_terrain_falls_back_high(failures)
...
func _test_missing_terrain_falls_back_high(failures: Array[String]) -> void:
	var provider: FlightAltitudeReferenceProvider = FlightAltitudeReferenceProvider.new()
	provider.reset_to_route_state(5, 0.0, 280.0, true)
	provider.update_from_terrain_sample(6, 0.4, 0.0, false, 0.1)
	expect_true(
		provider.mode == FlightAltitudeReferenceProvider.Mode.AGL
		and not provider.terrain_hit_valid
		and not provider.has_numeric_altitude()
		and is_equal_approx(provider.raw_terrain_altitude_meters, 0.0)
		...
		"An AGL ray miss must never leak a zero altitude to either consumer.",
		failures
	)
