...
`WhiteNoiseSideOrderContract` 是 T-126 白噪返件支线的单一变体合同。它集中声明
正式支线、五套独立对白、`17000 m` 起点、同一航路/抵达场景、三项选择、结尾值、
结果文案和 `70%` 关系阈值。注册表校验要求订单必须为 `PLAYABLE SIDE`、精确
依赖白噪主单且不得发放穹林章节或导航奖励。

`WhiteNoiseFlight` 在 `_ready()` 时从 active order 缓存是否为支线航段；支线从
段索引 `3` 的 `17096 m` 安全检查点恢复，HUD 进度按本单 `17000 m` 窗口重新
归一化。该缓存同时避免切换抵达场景后，旧 HUD 再查询已离树节点的
`/root/GameState`。驾驶舱、航路 HUD、冰下抵达区和结算只按稳定订单 ID 切换
局部 Resource/本地化 Key；物理航路、暴雪、有限平台和 1.5 屏区域保持同一份
实例，不创建支线专用关卡副本或全局 Manager。

大厅出发确认同样从当前活动 `OrderDefinition` 投影目的星球与货物显示名，不再
保存赤砂首单专用摘要。驾驶舱 `CockpitStarfield` 消费活动订单目的星球 ID：
赤砂保留暖色圆盘，白噪使用冰蓝表面、青绿大气辉光和独立配色签名；旅行控制器
仍只负责阶段/进度，不复制星球内容判断。

支线结算继续调用 `settle_current_order()` 的单一事务。货物完整度低于 `70%`
时传入白噪关系 `-1`，抵消订单基础 `+1`；三项选择只改变
`ending_white_noise_memory_return` 和已解锁人物/货物图鉴的描述投影。
`FAILED / ABANDONED` 沿用 T-103 状态机并释放 active 槽位，不修改已经成立的
白噪主单、穹林章节/导航、档案许可或模块。通用放弃确认与重接 UX 仍归 T-140。
...
Gate F Round 2 将近轨道 profile 的重力混合收敛为 `0.04`，但仍从白噪
`PlanetDefinition` 读取 `256 px/s² / 1.28g` 权威重力；默认有限补偿持续推进时，
飞船能在 `4500 m` 地表建立前留在可见走廊。路线运行时另以
`Y=-160…700` 作为世界绘制安全边界，越界通过 `FlightLabShip.fail_flight()`
进入既有自动检查点恢复，而不是让相机继续跟随到空白世界。

冰原地表和 `32750–34000 m` 有限平台只使用开放 `SegmentShape2D` 顶面，平台
视觉长度仍为 `1250 m`，不使用封闭矩形产生前缘或尾缘侧墙。着陆只允许在
`33700–34000 m` 接触窗口和平台表面容差内成立；船体中心明显低于平台，或越过
`34000 m` 真实尾缘仍未接地时，HUD 显示本地化的错过进场原因，并从
`28500 m` 最终进场检查点自动恢复。
...
- `WhiteNoiseRouteVisuals` 继续拥有世界空间程序绘制，将深海蓝高空、冻结星球
  弧、远/中景冰层、碰撞冰面、半透明裂谷、绿粉极光、高压弧、档案光栅和有限
  平台组织为七层 production placeholder；它不读取存档、音频或危险结算。
- `WhiteNoiseEnvironmentFeedback` 是当前白噪场景的 `CanvasLayer`，管理屏幕
  雪粒和运行时合成的安全/暴雪/地下三层循环与短记忆动机。切段只交叉改变本地
  音量目标和雪粒强度，不创建全局音频 Manager，也不写玩家设置。

既有 `high_contrast_terrain` 同时作为白噪视觉降噪辅助：雪粒数量
`58 → 24`、世界暴雪线 `92 → 38`，屏幕干扰线约降为 `56%`，高压脉冲闪光
Alpha 乘 `0.42`。路线提示、HUD、碰撞轮廓和危险状态不随降噪关闭；安全、暴雪
与地下音频仍保持可辨。程序音频、程序绘制和仓库既有粒子纹理来源记录在
`ATTRIBUTION.md`。
...
可用/已安装模块、许可、关系、站点状态、图鉴、纪念品、目标 Stage、目标场景和
预览限制，并在应用前验证稳定 ID、章节对应的星球前缀、订单/星球引用、模块
槽位、收藏引用和隔离夹具组合。
...
“自动存档已关闭”，不建立玩家可见调试菜单。该紧凑状态框每次 scenario 启动或
`F6` 重置时显示，`5 s` 后只隐藏面板但保留上下文，避免覆盖随后出现的驾驶舱
设备与对白。`F6 / m1_debug_reset` 重新应用初始快照并重载目标场景，使运行时
数据和场景夹具同时回到确定状态。
...
机制、白噪冰下档案目的地、白噪正式结算/返站/存档闭环、白噪返件支线闭环、
Gate F 主单到自愿支线交接、十个 M1 调试启动和 M0 完整闭环；该入口由
...
Gate F 入口 `--m1-debug=gate_f` 从 Gate E 已完成后的 `STATION` 开始，精确保留
赤砂回访完成状态、当地保留记录的隔离分支、`240` 信用点、已通电档案终端、
回访图鉴、白噪导航与已安装特高压电屏蔽罩，但不预接白噪主单。白噪主单完成后
正式状态流才开放 `side_white_noise_returned_memory`；支线仍可放弃且不撤销
穹林章节/星球资格。`F6` 始终恢复主单接取前的同一快照，不能把主单后状态误当成
第二个存档槽。
