extends SceneTree

const TERMINAL_SCENE_PATH: String = "res://scenes/ui/order_terminal.tscn"
const FLIGHT_SCENE_PATH: String = (
	"res://scenes/flight/white_noise_flight.tscn"
)
const ARRIVAL_SCENE_PATH: String = (
	"res://scenes/arrival/white_noise_arrival.tscn"
)
const RESULTS_SCENE_PATH: String = "res://scenes/app/results.tscn"
const REGISTRY_PATH: String = "res://data/m1_data_registry.tres"
const CONTRACT_PATH: String = (
	"res://data/settlements/white_noise_side_order_contract.tres"
)
const CAPTURE_DIRECTORY: String = (
	"res://.omx/artifacts/t126_white_noise_side"
)

var _original_locale: String = ""
var _game_state: GameStateModel
var _registry: GameDataRegistry
var _contract: WhiteNoiseSideOrderContract


func _initialize() -> void:
	_original_locale = TranslationServer.get_locale()
	TranslationServer.set_locale("zh_CN")
	call_deferred("_capture_frames")


func _capture_frames() -> void:
	_game_state = root.get_node_or_null("GameState") as GameStateModel
	_registry = load(REGISTRY_PATH) as GameDataRegistry
	_contract = load(CONTRACT_PATH) as WhiteNoiseSideOrderContract
	if (
		_game_state == null
		or _registry == null
		or _contract == null
		or not _prepare_qualified_state()
	):
		_fail("Could not prepare the T-126 visual fixture.")
		return
	if not await _capture_optional_terminal():
		return
	if (
		not _game_state.accept_order(_contract.order)
		or not _game_state.confirm_departure(_contract.order)
	):
		_fail("Could not accept and configure the visual side order.")
		return
	if not await _capture_short_route():
		return
	if not await _capture_arrival_choice():
		return
	if not await _capture_results():
		return
	_restore_runtime()
	print(
		"[t126-visual] PASS: saved optional terminal, short-route HUD, "
		+ "private-return choice, and side results frames."
	)
	quit(0)


func _prepare_qualified_state() -> bool:
	_game_state.reset_runtime_state()
	_game_state.main_story_chapter = M1ProgressRules.CHAPTER_M1_CANOPY_WORLD
	_game_state.unlocked_planet_ids = [
		M1ProgressRules.PLANET_RED_SAND,
		M1ProgressRules.PLANET_WHITE_NOISE,
		M1ProgressRules.PLANET_CANOPY_WORLD,
	]
	_game_state.planet_permission_ids.append(
		M1ProgressRules.PERMISSION_WHITE_NOISE_ARCHIVE_ACCESS
	)
	for completed_order_id: StringName in [
		M1CatalogModel.M0_ORDER_ID,
		M1ProgressRules.ORDER_RED_SAND_SHIELDING_RETROFIT,
		&"order_m1_white_noise_archive_core",
	]:
		_game_state.completed_order_ids[completed_order_id] = true
		_game_state.order_states[
			completed_order_id
		] = GameStateModel.OrderStatus.COMPLETED
		_game_state.reward_applied_order_ids.append(completed_order_id)
	_game_state.set_story_flag(
		&"story_m1_white_noise_archive_core_completed"
	)
	var shielding: ShipModuleDefinition = _registry.find_module(
		M1ProgressRules.MODULE_HIGH_VOLTAGE_SHIELDING
	)
	if shielding == null:
		return false
	if not _game_state.ship_upgrade_ids.has(shielding.id):
		_game_state.ship_upgrade_ids.append(shielding.id)
	return (
		_game_state.equip_ship_module(shielding)
		and _game_state.can_accept_order(_contract.order)
	)


func _capture_optional_terminal() -> bool:
	var packed: PackedScene = load(TERMINAL_SCENE_PATH) as PackedScene
	var terminal: OrderTerminalUI = packed.instantiate() as OrderTerminalUI
	if terminal == null:
		_fail("Optional order terminal could not instantiate.")
		return false
	terminal.set_game_state_override(_game_state)
	terminal.set_data_registry(_registry)
	root.add_child(terminal)
	await _settle_frames(4)
	if (
		not terminal.open_terminal()
		or not terminal.select_order(_contract.order.id)
		or not terminal.get_feedback_text().contains("自愿支线")
	):
		_fail("Optional order terminal could not stage the side order.")
		return false
	await _settle_frames(3)
	var saved: bool = _save_frame("01_optional_order_terminal.png")
	await _cleanup_node(terminal)
	return saved


func _capture_short_route() -> bool:
	var packed: PackedScene = load(FLIGHT_SCENE_PATH) as PackedScene
	var route: WhiteNoiseFlight = packed.instantiate() as WhiteNoiseFlight
	if route == null:
		_fail("Short White Noise route could not instantiate.")
		return false
	route.game_state_override = _game_state
	root.add_child(route)
	await _settle_frames(5)
	route.set_process(false)
	route.get_flight_ship().set_physics_process(false)
	if (
		not route.is_side_order_route()
		or route.get_active_segment_index()
		!= _contract.route_start_segment_index
	):
		_fail("Short White Noise route did not start in its reused segment.")
		return false
	await _settle_frames(2)
	var saved: bool = _save_frame("02_short_route_hud.png")
	await _cleanup_node(route)
	return saved


func _capture_arrival_choice() -> bool:
	var run_state: OrderRunState = _game_state.get_active_order_run_state()
	if run_state == null:
		_fail("Side-order run state is unavailable for arrival.")
		return false
	run_state.cargo_integrity = 94.0
	run_state.record_landing_result(
		OrderRunState.LANDING_RESULT_SMOOTH,
		0.0
	)
	var packed: PackedScene = load(ARRIVAL_SCENE_PATH) as PackedScene
	var arrival: WhiteNoiseArrival = packed.instantiate() as WhiteNoiseArrival
	if arrival == null:
		_fail("Side-order arrival could not instantiate.")
		return false
	arrival.game_state_override = _game_state
	root.add_child(arrival)
	await _settle_frames(5)
	var dialogue_ui: DialogueUI = arrival.get_dialogue_ui()
	if (
		dialogue_ui == null
		or dialogue_ui.skip_dialogue_sequence()
		!= DialogueRuntime.SequenceSkipResult.STOPPED_AT_CHOICE
	):
		_fail("Side-order arrival choice could not be staged.")
		return false
	await _settle_frames(3)
	if not _save_frame("03_private_return_choice.png"):
		return false
	if (
		not dialogue_ui.select_choice(&"returned_memory_anonymous_index")
		or dialogue_ui.skip_dialogue_sequence()
		!= DialogueRuntime.SequenceSkipResult.FINISHED
	):
		_fail("Side-order arrival choice could not finish.")
		return false
	await _settle_frames(3)
	arrival.queue_free()
	await _settle_frames(3)
	return true


func _capture_results() -> bool:
	var packed: PackedScene = load(RESULTS_SCENE_PATH) as PackedScene
	var results: OrderResults = packed.instantiate() as OrderResults
	if results == null:
		_fail("Side-order results could not instantiate.")
		return false
	results.game_state_override = _game_state
	root.add_child(results)
	await _settle_frames(5)
	if not results.is_settlement_committed():
		_fail("Side-order results did not commit.")
		return false
	var saved: bool = _save_frame("04_anonymous_index_results.png")
	await _cleanup_node(results)
	return saved


func _settle_frames(frame_count: int) -> void:
	for _frame_index: int in frame_count:
		await process_frame


func _cleanup_node(node: Node) -> void:
	if node != null and is_instance_valid(node):
		node.queue_free()
		await _settle_frames(3)


func _save_frame(file_name: String) -> bool:
	var absolute_directory: String = ProjectSettings.globalize_path(
		CAPTURE_DIRECTORY
	)
	if DirAccess.make_dir_recursive_absolute(absolute_directory) != OK:
		_fail("Could not create the T-126 capture directory.")
		return false
	var image: Image = root.get_texture().get_image()
	if image == null or image.is_empty():
		_fail("T-126 viewport capture was empty.")
		return false
	if image.get_size() != Vector2i(640, 360):
		image.resize(640, 360, Image.INTERPOLATE_NEAREST)
	var output_path: String = absolute_directory.path_join(file_name)
	if image.save_png(output_path) != OK:
		_fail("Could not save %s." % output_path)
		return false
	print("[t126-visual] Saved %s" % output_path)
	return true


func _restore_runtime() -> void:
	if _game_state != null:
		_game_state.reset_runtime_state()
	TranslationServer.set_locale(_original_locale)


func _fail(message: String) -> void:
	printerr("[t126-visual] FAIL: %s" % message)
	_restore_runtime()
	quit(1)
