...
按路线进度统一计算位置、尺度和透明度；阶段 4 以 `0.90 s` 将圆盘放大、下移
并交叉过渡到统一 `AtmosphereHorizon` 曲面层；阶段 5 起强制隐藏完整圆盘，
低层云再逐步淡出曲面并交给远景地形和设施视差。重开或检查点恢复调用同一
`reset_to_distance()`，不维护互相独立的 Tween。

`FlightRouteSegment.terrain_surface_enabled` 同时门控该段的 `FloorVisual`、
`FloorEdge` 与 `FloorBody`。赤砂星阶段 1–5 保持关闭，只保留背景和视觉危险；
低层云、峡谷地表与着陆段才打开真实地表。`floor_y` 在前段只作为压缩 HUD
高度基准，不得隐式产生碰撞。
...
T-042 使用局部 `RedSandLowFlightCourse` 组合手工固定的峡谷/设施
`StaticBody2D`、三个 `FlightRadarSector` 和与岩檐、管线、设施门对应的
`FlightRadarCover`。雷达只以 `FlightRadarModel` 根据高度、固定扇区和遮挡
显式积分锁定风险，不使用敌人、武器或潜行 AI；高空直线路径保持开放但会快速
累积锁定，低空引导线经过需要减速修正的遮挡区并使锁定衰减。每个检查点首次
完成锁定会请求一次确定性环境脉冲，当前由 `FlightLabShip` 既有资源接口扣除
`12` 点护盾/船体与 `2` 点货物，并提供本地化状态和轻微镜头反馈；保持在 HUD
安全高度内则完全避免同一后果，不新增敌人攻击或战斗状态机。
既有路线提示开关控制青色安全路线与遮挡轮廓，高对比度地形开关额外强化所有
碰撞多边形边缘；两者均保持输入穿透。T-044 已用地表尘粒、动态危险音乐和
设施视差补足首轮环境反馈；正式地形素材与专用雷达音色留待后续美术迭代。

雷达课程在路线距离 `32650` 处锁存为 `LANDING_BUFFER`，清空扫描压力；着陆台
可碰撞边缘约从 `35600` 开始，中间约 `2950` 的确定性走廊用于抬升、减速和
重新对准。进入最终段后雷达清除提示与着陆引导可同时显示但不得重叠。
...
进入最终阶段时，`FlightLabShip` 以固定安全位置和速度替换重试快照，但保留当时
的环境、辅助驾驶和资源状态；失败沿用现有 `flight_restart` 与快速自动重试，
只回到着陆进场。成功后飞船停止受控运动，勉强着陆只扣除少量货物完整度，结果
